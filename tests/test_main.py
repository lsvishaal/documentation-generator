...
from documentation_generator.main import handler
...
async def test_handler_returns_response():
    ...
    with patch("documentation_generator.main._initialized", True), \
         patch("documentation_generator.main.run_agent", new_callable=AsyncMock, return_value=mock_response):
...
async def test_handler_with_multiple_messages():
    ...
    with patch("documentation_generator.main._initialized", True), \
         patch("documentation_generator.main.run_agent", new_callable=AsyncMock, return_value=mock_response) as mock_run:
        result = await handler(messages)
...
async def test_handler_initialization():
    ...
    with patch("documentation_generator.main._initialized", False), \
         patch("documentation_generator.main.initialize_all", new_callable=AsyncMock) as mock_init, \
         patch("documentation_generator.main.run_agent", new_callable=AsyncMock, return_value=mock_response), \
         patch("documentation_generator.main._init_lock"):
